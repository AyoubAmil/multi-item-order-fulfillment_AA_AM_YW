...
class FulfillmentPolicy:
    def __init__(self, num_items=20, n_max=5, n_0=5, p_stock=0.75, T=10**4, CSL=0.5, facilities_data="Data/fulfillment_centers_warmup.csv", cities_data="Data/cities_warmup.csv", prob_seed_value=1, order_seed_value=1, inv_seed_value=1, alpha=0.5):
        ...
        self.order_fulfillment = OrderFulfillment(num_items, n_max, n_0, p_stock, T, CSL, facilities_data, cities_data, prob_seed_value, order_seed_value, inv_seed_value, alpha)
        ...
        self.LP_solution, self.methods, self.sizes = self.solving_LP_instance.optimize_LP_relaxation()
        # Define probabilities for magician problems
        self.consumption_probability_lists = self.solving_LP_instance.calculate_probabilities_of_consumption(self.LP_solution, self.sizes)
    ...
    def fulfillment_policy(self, inventory_consumption, magician_problems, seed_value):
        ...
        def check_magician_open(pair_i_k, t):
            ...
            return magician_problems[pair_i_k]['open_list'][t][opening_index] == 0 # magician does not open the box
