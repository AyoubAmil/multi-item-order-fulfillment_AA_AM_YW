...
class OrderFulfillment:
    def __init__(self, num_items=20, n_max=5, n_0=5, p_stock=0.75, T=10**4, CSL=0.5, facilities_data="Data/fulfillment_centers_warmup.csv", cities_data="Data/cities_warmup.csv", prob_seed_value=1, order_seed_value=1, inv_seed_value=1):
        ...
        self.expected_demand_k_i = self.calculate_expected_demand(self.facility_indices, self.items, self.location_customers_by_facility_and_item, self.order_types, self.demand_distribution_by_type_by_location) # Expected demand for each facility and item
        ...
        self.agg_demand_distribution_by_type_by_location = self.aggregate_demand_distribution() # Aggregate (across time) demand distribution by type by location
        self.reshape_arrival_prob = self.reshape_probs(self.demand_distribution_by_type_by_location) # Reshape the demand distribution
        self.reshape_agg_arrival_prob = self.reshape_probs(self.agg_demand_distribution_by_type_by_location) # Reshape the aggregate demand distribution
    ...
    def validate_attributes(self):
        necessary_attributes = ['num_items', 'n_max', 'n_0', 'p_stock', 'T', 'CSL', 'facilities_data', 
                                ...
                                'demand_distribution_by_type_by_location', 'df_orders', 
                                'df_orders_location', 'data_inventory', 'df_inventory', 
                                'location_customers_by_facility_and_item', 'expected_demand_k_i', 
                                'safety_stock', 'agg_demand_distribution_by_type_by_location', 'reshape_arrival_prob', 
                                'reshape_agg_arrival_prob', 'all_methods_location', 'all_costs', 
                                'all_indicators']
    ...
    def calculate_expected_demand(self, facility_indices, items, location_customers_by_facility_and_item, order_types, demand_distribution_by_type_by_location):
        """
        Calculate the total incoming expected demand for each facility and item.
        """
        expected_demand_k_i = []
                ...
                for j in location_customers_by_facility_and_item[k][i]:
                    ...
                    for order in order_types:
                        if i in order:
                            ...
                            # Add the probability of the order occurring at city j
                            prob += demand_distribution_by_type_by_location[len(order)][index_order-self.n_0*(len(order)-1)-1][j]
                expected_demand_k_i.append(prob)
        return expected_demand_k_i
    
    # Calculate safety stock for each facility k and item i
    def calculate_safety_stock(self, facility_indices, items, T, expected_demand_k_i, CSL):
        """
        Calculate safety stock based on the expected demand and service level.
        """
        mu = []
        sd = []
        for k in facility_indices:
        ...
        for k in facility_indices:
            S_k = []
            for i in items:
                S_k.append(np.ceil(mu[k*len(items)+i] + sps.norm.ppf(CSL, loc=0, scale=1) * sd[k*len(items)+i]))
            S.append(S_k)
        return S
    ...
    # Aggregate demand distribution over time (assuming iid demand)
