...
class FulfillmentPolicy:
    def __init__(self, num_items=20, n_max=5, n_0=5, p_stock=0.75, T=10**4, CSL=0.5, facilities_data="Data/fulfillment_centers_warmup.csv", cities_data="Data/cities_warmup.csv", prob_seed_value=1, order_seed_value=1, inv_seed_value=1):
        # Initialize OrderFulfillment object with the desired parameters
        self.order_fulfillment = OrderFulfillment(num_items, n_max, n_0, p_stock, T, CSL, facilities_data, cities_data, prob_seed_value, order_seed_value, inv_seed_value)
        ...
        # Create SolvingLP object with the OrderFulfillment object
        self.solving_LP_instance = SolvingLP(self.order_fulfillment)
        # Get solution to LP and the respective methods
        self.LP_solution, self.methods = self.solving_LP_instance.optimize_LP_relaxation()
        # Define probabilities for magician problems
        self.probabilities = self.solving_LP_instance.calculate_probabilities_of_consumption(self.LP_solution)
        self.consumption_probability_lists = self.solving_LP_instance.generate_consumption_probability_lists(self.probabilities)
        
        # Obtain optimization results
        self.get_optimization_results = self.solving_LP_instance.get_optimization_results()
    ...
    def probabilistic_fulfillment(self):
        # sampled order
        sampled_order = self.sample_order_index(self.order_fulfillment.reshape_agg_arrival_prob)
        # method distribution conditional on sampled order (LP_solution[sampled_order] is aggregate solution so proper constant is 1/aggregate arrival probability)
        sample_method_dict = self.dict_mult(self.LP_solution[sampled_order], 1/self.order_fulfillment.reshape_agg_arrival_prob[sampled_order])
    ...
    def fulfillment_policy(self, inventory_consumption, magician_problems, seed_value):
        ...
        for t in range(self.order_fulfillment.T):
            sampled_order, sampled_method, method_index = self.probabilistic_fulfillment()
            accept = True
    ...
    def always_accept_policy(self, inventory_consumption, seed_value):
        ...
        for t in range(self.order_fulfillment.T):
            sampled_order_aa, sampled_method_aa, method_index_aa = self.probabilistic_fulfillment()
            accept_aa = True
