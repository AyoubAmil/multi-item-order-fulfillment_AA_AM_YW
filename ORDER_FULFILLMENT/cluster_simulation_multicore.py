...
def evaluate_policies_subset(n_max, T, alpha, instance, conservative_prob_subset):
    ...
    for conservative_prob in conservative_prob_subset:
        ...
        # 95% CONFIDENCE INTERVAL for the difference (for fixed instance)
        ...
        results_per_conservative_probs = {
            ...
            "expected_cost_difference_over_order_sequence": expected_cost_difference,
    ...
    # Create a unique identifier for the subset
    subset_identifier = "-".join([f"{prob:.3f}" for prob in conservative_prob_subset])
    
    res_path = f"/hpc/home/aa554/Results/multicore/num_items={num_items}_n_max={n_max}_n_0={n_0}/instance={instance}/T={T}/alpha={alpha}/subset={subset_identifier}"
    
    if not os.path.exists(res_path):
        os.makedirs(res_path)

    # Define the CSV file path
    csv_file_path = os.path.join(res_path, f'instance={instance}_T={T}_alpha={alpha}_subset={subset_identifier}.csv')
...
def worker(n_max, T, alpha, instance, conservative_prob_subset):
    # This is a worker function for multiprocessing
    evaluate_policies_subset(n_max, T, alpha, instance, conservative_prob_subset)
...
if __name__ == "__main__": 
    ...
    # Split conservative_prob_sequence into chunks for each core
    num_cores = 4
    chunks = [conservative_prob_sequence[i::num_cores] for i in range(num_cores)]

    # Create a pool of processes and distribute the workload
    pool = multiprocessing.Pool(processes=num_cores)
    for i in range(num_cores):
        pool.apply_async(worker, args=(n_max, T, alpha, instance, chunks[i]))
    pool.close()
    pool.join()